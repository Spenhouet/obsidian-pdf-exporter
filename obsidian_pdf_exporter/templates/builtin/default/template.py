"""Generic, brand-neutral default template.

This template demonstrates the three responsibilities every template can
implement:

1. **Styling** — :meth:`get_css` returns the bundled CSS file.
2. **Decorations** — :meth:`decorations` returns running header/footer HTML
   plus an ``@page`` rule block. The same mechanism a downstream branded
   template would use to inject a corporate logo, colour palette or
   confidentiality notice.
3. **Hooks** — left as no-ops here, but a downstream template could mutate
   the assembled markdown or post-pandoc HTML in :meth:`process_markdown` /
   :meth:`process_html`.
"""
...
class DefaultTemplate(Template):
    """Clean, brand-neutral A4 portrait template with title header and page footer."""

    name = "default"
    description = "Generic A4 documentation template with title header and page footer"

    def get_css(self) -> str:
        return resources.files(__package__).joinpath("default.css").read_text(encoding="utf-8")
    ...
    def decorations(self, context: ExportContext) -> Decorations | None:
        ...
        label_css = _css_str(
            f"Document version: {context.label}" if context.label else "",
        )
