...
This document is the full reference for authoring PDF templates for `obsidian-pdf-exporter`. For a high-level overview see the [Templates section in the README](README.md#templates).
...
Templates are *brand* concerns. Markdown/Obsidian semantics belong in plugins, not templates.
...
| `blockquote` | Quotes (note: Obsidian callouts are *not* blockquotes after conversion). |
...
- **CSS strings are *not* Python f-strings.** When you build a `content: "…"` value with an f-string, escape quotes and newlines (use the `_css_str()` helper). Forgetting this is the #1 source of WeasyPrint syntax errors.
